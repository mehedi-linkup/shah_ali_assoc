const billInvoice = Vue.component('bill-mutiple-invoice', {
    template: `
        <div>
            ...
            <div id="invoiceContent">
                <div v-for="row in cart" class="continer-brack">
                <div class="row">
                ...
                <div class="row">
                    <div class="col-xs-7">
                        <strong>দোকান / স্পেস নংঃ </strong> {{ convertToBanglaNumber(row?.Store_No) }} {{ row?.Floor_Name }}<br>
                        <strong>সদস্য / আইডি নংঃ </strong> {{ row?.Owner_Code }}<br>
                        <strong>নামঃ </strong> {{ row?.Owner_Name }}<br>
                    </div>
                    <div class="col-xs-5 text-right">
                        <strong>মাসঃ </strong> {{ row.month_name }}<br>
                        <strong>তারিখঃ </strong> {{ startDate }} হইতে {{ endDate }} পর্যন্ত <br>
                        <strong>আয়তন</strong> {{ convertToBanglaNumber(row.square_feet) }}<br><br>
                      
                        <strong>বিচ্ছিন্ন করণের তারিখঃ </strong> {{ dateFormat(row?.last_date) }}
                    </div>
                </div>
                        ...
                        <table _a584de>
                            ...
                            <tbody>
                                <tr v-if="row.savings_deposit > 0">
                                    ...
                                    <td style="text-align:right">{{ convertToBanglaNumber(row?.savings_deposit) }}</td>
                                </tr>
                                <tr v-if="row.membership_fee > 0">
                                    ...
                                    <td style="text-align:right">{{ convertToBanglaNumber(row?.membership_fee) }}</td>
                                </tr>
                                <tr>
                                    <td style="text-align:left">{{ row.membership_fee == 0? '১':'৩'}}</td>
                                    <td style="text-align:left">{{ convertToBanglaNumber(301005) }}</td>
                                    <td style="text-align:left">দোকান ভাড়া</td>
                                    <td style="text-align:right">{{ convertToBanglaNumber(row?.shop_rent) }}</td>
                                </tr>
                                <tr>
                                    <td style="text-align:left">{{ row.membership_fee == 0? '২':'৪'}}</td>
                                    <td style="text-align:left">{{ convertToBanglaNumber(301006) }}</td>
                                    <td style="text-align:left">কর সারচার্জ</td>
                                    <td style="text-align:right">{{ convertToBanglaNumber(row?.tax_surcharge) }}</td>
                                </tr>
                                <tr>
                                    <td style="text-align:left">{{ row.membership_fee == 0? '৩':'৫'}}</td>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align:right">{{ convertToBanglaNumber(row?.net_payable) }} </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div class="col-xs-6">
                        <table _t92sadbc2>
                            ...
                            <tr>
                                ...
                                <td style="text-align:right">{{ convertToBanglaNumber(row?.late_fee) }}</td>
                            </tr>
                            <tr><td colspan="2" style="border-bottom: 1px solid #ccc"></td></tr>
                            <tr>
                                <td><strong>বিলম্ব ফি সহ সর্বমোট:</strong></td>
                                <td style="text-align:right">{{ convertToBanglaNumber(parseFloat( +row?.previous_due + +row?.net_payable + +row?.late_fee).toFixed(2) ) }}</td>
                            </tr>
                        </table>
                    </div>
                </div>
    ...
    methods:{
        getbill(){
            axios.post('/get_zamindari_bill_details', {floorId: this.floor_id, monthId: this.month_id}).then(res=>{
