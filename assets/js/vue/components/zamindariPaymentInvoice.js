const billInvoice = Vue.component('billInvoice', {
    template: `
        <div>
            ...
            <div id="invoiceContent">
                ...
                <div class="row">
                    <div class="col-xs-7">
                        <strong>দোকান / স্পেস নংঃ </strong> {{ convertToBanglaNumber(payment?.Store_No) }} {{ payment?.Floor_Name }}<br>
                        <strong>সদস্য / আইডি নংঃ </strong> {{ payment?.Owner_Code }}<br>
                        <strong>নামঃ </strong> {{ payment?.Owner_Name }}<br>
                    </div>
                    <div class="col-xs-5 text-right">
                        <strong>মাসঃ </strong> {{ payment.month_name }}<br>
                        <strong>তারিখঃ </strong> {{ startDate }} হইতে {{ endDate }} পর্যন্ত <br>
                        <strong>আয়তন</strong> {{ convertToBanglaNumber(payment.square_feet) }}<br><br>
                      
                        <strong>বিচ্ছিন্ন করণের তারিখঃ </strong> {{ dateFormat(payment?.last_date) }}
                    </div>
                </div>
                <div class="row">
                    <div class="col-xs-12">
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <table _a584de>
                            ...
                            <tbody>
                                <tr v-if="payment.is_member == 'true'">
                                    ...
                                    <td style="text-align:right">{{ convertToBanglaNumber(payment?.savings_deposit) }}</td>
                                </tr>
                                <tr v-if="payment.is_member == 'true'">
                                    ...
                                    <td style="text-align:right">{{ convertToBanglaNumber(payment?.membership_fee) }}</td>
                                </tr>
                                <tr>
                                    <td style="text-align:left">{{ payment.is_member == 'false'? '১':'৩'}}</td>
                                    <td style="text-align:left">{{ convertToBanglaNumber(301005) }}</td>
                                    <td style="text-align:left">দোকান ভাড়া</td>
                                    <td style="text-align:right">{{ convertToBanglaNumber(payment?.shop_rent) }}</td>
                                </tr>
                                <tr>
                                    <td style="text-align:left">{{ payment.is_member == 'false'? '২':'৪'}}</td>
                                    <td style="text-align:left">{{ convertToBanglaNumber(301006) }}</td>
                                    <td style="text-align:left">কর সারচার্জ</td>
                                    <td style="text-align:right">{{ convertToBanglaNumber(payment?.tax_surcharge) }}</td>
                                </tr>
                                <tr>
                                    <td style="text-align:left">{{ payment.is_member == 'false'? '৩':'৫'}}</td>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align:right">{{ convertToBanglaNumber(payment?.net_payable) }} </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div class="col-xs-6">
                        <table _t92sadbc2>
                            ...
                            <tr>
                                ...
                                <td style="text-align:right">{{ convertToBanglaNumber(parseFloat( +payment?.net_payable + +0).toFixed(2) ) }}</td>
                            </tr>
                        </table>
                    </div>
                </div>
    ...
    methods:{
        getPayment(){
            axios.post('/get_zamindari_payments', {paymentId: this.payment_id}).then(res=>{
