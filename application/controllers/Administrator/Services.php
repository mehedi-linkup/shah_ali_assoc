...
class Services extends CI_Controller {
    ...
    public function index()  {
        ...
        $data['title'] = "Service Utility";
        $data['serviceCode'] = $this->mt->generateServiceCode();
        $data['content'] = $this->load->view('Administrator/services/add_service', $data, TRUE);
    ...
    public function addService() {
        ...
        try{
            ...
            $serviceNameCount = $this->db->query("select * from tbl_service where Service_Name = ?", $serviceObj->Service_Name)->num_rows();
            if($serviceNameCount > 0){
                $res = ['success'=>false, 'message'=>'Service already exists'];
                echo json_encode($res);
                exit;
            }

            $serviceCodeCount = $this->db->query("select * from tbl_service where Service_Code = ?", $serviceObj->Service_Code)->num_rows();
            if($serviceCodeCount > 0){
                $res = ['success'=>false, 'message'=>'Service code already exists'];
                echo json_encode($res);
                exit;
            }
            ...
            $service['is_service'] = $serviceObj->is_service == true ? 'true' : 'false';
            $service['status'] = 'a';
            $service['AddBy'] = $this->session->userdata("FullName");
            $service['AddTime'] = date('Y-m-d H:i:s');
            $service['Service_branchid'] = $this->brunch;

            $this->db->insert('tbl_service', $service);
            // print_r($this->db->error());
            // return;

            $res = ['success'=>true, 'message'=>'Service added successfully', 'serviceId'=>$this->mt->generateServiceCode()];
        } catch (Exception $ex){
    ...
    public function updateService(){
        ...
        try{
            ...
            $serviceNameCount = $this->db->query("select * from tbl_service where Service_Name = ? and Service_SlNo != ?", [$serviceObj->Service_Name, $serviceObj->Service_SlNo])->num_rows();
            if($serviceNameCount > 0){
                $res = ['success'=>false, 'message'=>'Service already exists'];
                echo json_encode($res);
                exit;
            ...
            $serviceCodeCount = $this->db->query("select * from tbl_service where Service_Code = ? and Service_SlNo != ?", [$serviceObj->Service_Code, $serviceObj->Service_SlNo])->num_rows();
            if($serviceCodeCount > 0){
                $res = ['success'=>false, 'message'=>'Service code already exists'];
                echo json_encode($res);
                exit;
            ...
            $service = (array)$serviceObj;
            unset($service['Service_SlNo']);
            $service['is_service'] = $serviceObj->is_service == true ? 'true' : 'false';
            $service['UpdateBy'] = $this->session->userdata("FullName");
            $service['UpdateTime'] = date('Y-m-d H:i:s');

            $this->db->where('Service_SlNo', $serviceObj->Service_SlNo)->update('tbl_service', $service);

            $res = ['success'=>true, 'message'=>'Service updated successfully', 'serviceId'=>$this->mt->generateServiceCode()];
        } catch (Exception $ex){
            $res = ['success'=>false, 'message'=>$ex->getMessage()];
        }
    ...
    public function deleteService(){
        ...
        try{
            ...
            $this->db->set(['status' => 'd'])->where('Service_SlNo', $data->serviceId)->update('tbl_service');

            $res = ['success'=>true, 'message'=>'Service deleted successfully'];
        } catch (Exception $ex){
            $res = ['success'=>false, 'message'=>$ex->getMessage()];
        }
    ...
    public function getServices(){
        ...
        if(isset($data->categoryId) && $data->categoryId != ''){
            $clauses .= " and s.ServiceCategory_ID = '$data->categoryId'";
        }

        if(isset($data->isService) && $data->isService != null && $data->isService != ''){
            $clauses .= " and s.is_service = '$data->isService'";
        }

        $products = $this->db->query("
            select
                s.*,
                concat(s.Service_Name, ' - ', s.Service_Code) as display_text,
                u.Unit_Name
            from tbl_service s
            left join tbl_unit u on u.Unit_SlNo = s.Unit_ID
            where s.status = 'a'
            $clauses
            order by s.Service_SlNo desc
        ")->result();
