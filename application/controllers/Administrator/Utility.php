    ...
    public function addUtilityPayment()
    {
        ...
        try{
            ...
            $utility = array(
                ...
                'total_payment_amount' => $data->payment->total_payment_amount,
                'status' => 'a',
            ...
            foreach($data->storeBills as $utility){
                $utilityDetails = array(
                    ...
                    'bill_detail_id' => $utility->id,
                    'store_id' => $utility->store_id,
                    'electricity_bill' => $utility->electricity_bill_payment,
                    'generator_bill' => $utility->generator_bill_payment,
                    'ac_bill' => $utility->ac_bill_payment,
                    'others_bill' => $utility->others_bill_payment,
                    'late_fee' => $utility->late_fee,
                ...
                $transactionCount = $this->db->query("select * from tbl_transaction where store_id = ? and month_id = ? and branch_id = ?", [$utility->store_id, $data->payment->month_id, $this->session->userdata('BRANCHid')])->num_rows();
                if($transactionCount == 0){
                    $transaction = array(
                        'store_id' => $utility->store_id,
                        'month_id' => $data->payment->month_id,
                        'electricity_bill' => $utility->electricity_bill,
                        'electricity_payment' => $utility->electricity_bill_payment,
                        'generator_bill' => $utility->generator_bill,
                        'generator_payment' => $utility->generator_bill_payment,
                        'ac_bill' => $utility->ac_bill,
                        'ac_payment' => $utility->ac_bill_payment,
                        'others_bill' => $utility->others_bill,
                        'others_payment' => $utility->others_bill_payment,
                        'net_payable' => $utility->net_payable,
                        'total_payment' => $utility->payment,
                        'branch_id' => $this->session->userdata('BRANCHid')
                    );
                    ...
                    $this->db->query("
                        ...
                        SET 
                            ...
                            generator_bill = generator_bill + ?, 
                            generator_payment = generator_payment + ?, 
                            ac_bill = ac_bill + ?, 
                    ...
                    ", [
                        ...
                        $utility->electricity_bill_payment, 
                        $utility->generator_bill, 
                        $utility->generator_bill_payment, 
                        $utility->ac_bill, 
                        $utility->ac_bill_payment, 
                        $utility->others_bill, 
                        $utility->others_bill_payment, 
                        $utility->net_payable, 
                        $utility->payment, 
                        $utility->store_id,
            ...
            $res=['success'=>true, 'message'=>'Payment Success', 'utilityDetailsArr'=> $utilityDetailsArr];
        } catch (Exception $ex){
            $res = ['success'=>false, 'message'=>$ex->getMessage()];
        }
